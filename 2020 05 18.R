...
#
# - 자료 특성에 따른 분류 
#   부석 대상 자료의 특성에 따라 범주형 자료와 연속형 자료로 분류한다. 
# 
# 1. 범주형 자료( categotical data ) :                                factor type - 범주형 자료 
#     - 범주 또는 그룹으로 구분할 수 있는 값 
#     - 범주형 자료의 값은 기본적으로 숫자로 표현할 수 없고, 
#       대소비교나 산술연산이 적용되지 않는다. - 의미가 없다 
...
#
# 변수 개수에 따라
#   1. 단일변수 자료( univariate date ), 일변량 자료
#      - 하나의 변수로 구성된 자료 
...
#   범주형 자료에 할 수 있는 기본 작업 : 자료에 포함된 관측값들의 종류별로 개수를 세는 것 -> 도수분포표 
#                                                                 종류별로 비율을 구하는 것, 
#                                                        시각화는 막대/원 그래프를 사용 
#분석 - 변수, MR/DR - 변수 = 특성
#
...
barplot(ds, main = 'favorite season')        # 전체적인 면적을 보고 싶을 때 

ds.new <- ds[ c(2, 3, 1, 4) ]
ds.new
barplot( ds.new, main = 'favorite season' )  # 알파벳 순 

pie( ds, main = 'favorite season' )          # 전체적인 비율을 보고 싶을 때 
...
pie( ds, main = 'favorite season',
        col = colors )                             # x-범주형 데이터, y-도수 =막대그래프 




#
# 2.4 단일 변수 연속형 자료 탐색 - 범주형에 비해 분석하기가 까다롭다 
# 
# 단일 변수 연속형 자료 : 특성이 하나이면서 자료의 특성이 연속형인 자료
#                         연속형 자료는 관측값들이 크기를 가지므로 다양한 분석방법 존재
#
# 1. 평균(mean) : 하나의 값으로 전체를 대표할 수 있는 값, 이상값에 영향을 받는다. - 데이타가 치우칠수있음
#
...
#
# 4. 사분위수(quatrile) : 주어진 자료에 있는 값들을 크기순으로 나열 했을 때 4등분하는 지점에 있는 값 
#           1사분위수(Q1), 2사분위수(Q2, 중앙값과 동일), 3사분위수(Q3)
#           전체 자료를 4개로 나누므로 4개 구간은 25%의 자료가 존재 
#           - 최소값, 최대값을 벗어나는 값 : 이상치 -> 전처리 과정에서 뺄지 말지 정함 
# 
...
quantile(weight.heavy, (0:10) / 10)
    ...
    # 0.1 ~ 1.0사이 백분율로 1.% ~ 100%

summary(weight.heavy) # 한번에 알게 해 주는 함수 
...
boxplot.stats(dist) # 구체적인 값을 파악 할 때 사용 
                    ...
                    # $conf : 중앙값에 대한 신뢰구간 
                    # $out : 특이값( 이상치 ) 목록 
hist( dist, main = 'Histogram for 제동거리',          # 면적은 중요하지 않다, 구간이기 때문에 
      xlab = '제동거리', ylab = '빈도수',
      border = 'blue', col = 'green',
      las = 2, breaks =5 )
# 막대그래프&히스토그램 - 도수를 구하는 것 / 막대그래프는 면적이 중요 / 히스토그램은 면적 중요x

# 상자 그림(상자 그래프,  boxplot, 상자 수염 그래프 )
#   사분위수를 그래프형태로 시각화하는 도구 
#   상자그래프는 하나의 그래프로 데이터의 분포 형태를 포함한 다양한 정보를 전달 
#    - 자료의 전반적인 분포를 이해하는데 도움 
#    - 구체적인 최소/최대/중아값을 알기는 어렵다. - 구체적인 값을 알려면 summary() / boxplot.stats() 사용
boxplot(dist, main = '자동차 제동거리') 

# 오늘 하는 모든 건 데이터 이해 - 빅데이터는 데이터를 이해하고 그 이면을 분석하는 게 목적 / 최신 데이타가 아닌
# 이미 만들어져 있는 데이터를 이용해 그 안에 숨겨져 있는 의미를 찾아내는 일 = 빅데이터  

boxplot.stats(dist)
boxplot.stats(dist)$stats # 정상범위 측정
...
boxplot(iris$Petal.Length~iris$Species,    # 그룹정보형은 항상 마지막에 나와야 한다 
        main = '품종별 꽃잎 길이' )

# 한 화면에 여러 그래프 작성
par(mfrow=c(1, 3))     # 1 X 3 가상화면 분할 / 1줄에 그래프 3개 그려라 
